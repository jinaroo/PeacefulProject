...
namespace WhalexNew
{
    ...
    public class Controller2D : RaycastController
    {
        ...
        void HorizontalCollisions(ref Vector2 moveAmount)
        {
            ...
            {
                // new: this multiplier was increased to account for slow movement on very slight slopes
            ...
            for (int i = 0; i < horizontalRayCount; i++)
            {
                ...
                // new: casting all rays to deal with overlapped one-way platform
                foreach (RaycastHit2D hit in hits)
                {
                    ...
                    {
                        // new: ignore horizontal collisions on one-way platforms if we're trying to fall through them
                        continue;
                    }
                    ...
                    if (i == 0 && slopeAngle <= maxSlopeAngle)
                    {
                        ...
                        ) // new: prevent it resets to false accidentally
                        {
                    ...
                    //		OR
                    //		we're not shooting out of our feet (i != 0) and we're hitting a collider that we aren't standing on
                    // in other words, we want to climb the one-way slope which is less than 90 degree even when we don't press the down button
                    ...
                    if ((!collisions.climbingSlope || slopeAngle > maxSlopeAngle) && !ignoreHorizontalCollider)
                    {
                        ...
                        // when you are climbing a slope and run into a wall, you need this chunk to adjust y movement to avoid jittering
                        if (collisions.climbingSlope)
                        {
        ...
        void VerticalCollisions(ref Vector2 moveAmount)
        {
            ...
            for (int i = 0; i < verticalRayCount; i++)
            {
                ...
                RaycastHit2D[] hits =
                    Physics2D.RaycastAll(rayOrigin, Vector2.up * directionY, rayLength, collisionMask);
                ...
                foreach (RaycastHit2D hit in hits)
                {
                    if (hit.collider.CompareTag("Through"))
                    {
                        ...
                        // for getting through moving platform (legacy, waiting to be re-implemented)
            ...
            if (collisions.climbingSlope)
            {
                ...
                foreach (RaycastHit2D hit in hits)
                {
                    // ignore rays that are cast inside a collider
                    if (hit.distance != 0f)
                    {
                        float slopeAngle = Vector2.Angle(hit.normal, Vector2.up);
                ...
                for (int i = 0; i < verticalRayCount; i++)
                {
                    ...
                    foreach (RaycastHit2D hit in hits)
                    {
                        if (hit.collider.tag == "Through"
                            // new: 
                            && hit.distance != 0 && hit.collider == collisions.curGroundCollider)
                        {
                        ...
                        if (slopeAngle != 0 && slopeAngle <= maxSlopeAngle)
                        {
                            if (Mathf.Sign(hit.normal.x) == directionX)
                            {
                                if (hit.distance - skinWidth <= Mathf.Tan(slopeAngle * Mathf.Deg2Rad) *
                                    ...
                                    Mathf.Max(Mathf.Abs(moveAmount.x), skinWidth))
                                {
                                    float moveDistance = Mathf.Abs(moveAmount.x);
